defmodule Tremorx.Components.AreaChart do
  @moduledoc """
  Renders AreaChart
  """
  ...
  attr :id, :string, default: "areachart", doc: "The id of the chart"
  ...
  attr :x_axis_type, :string,
    default: "date",
    values: ~w(linear time date),
    doc: "The type of the x axis values. Can be 'linear', 'time' or 'date'"

  attr :x_axis_format, :string,
    default: "%b %Y",
    doc: "A format to apply to the x axis values. See d3-format for more information"
  ...
  attr :x_axis_ticks, :integer, default: nil, doc: "The minimum number of ticks on the x axis"
  ...
  @doc """
  An area chart displays quantitative data graphically.
  It is based on the line chart but the area between x-axis and line is emphasized with color.
  ...
  def area_chart(assigns) do
    ...
    <div id={@id} class="pr-0 w-full h-full">
      <div
        ...
        data-index={@index}
        data-x-axis-type={@x_axis_type}
        data-x-axis-format={@x_axis_format}
        data-y-axis-format={@y_axis_format}
        data-y-axis-ticks={@y_axis_ticks}
        data-x-axis-ticks={@x_axis_ticks}
        class={
          Tails.classes([
            Theme.make_class_name("areachart", "root"),
            "relative",
  ...
  defp build_categories(categories, colors) do
    ...
    |> Enum.map(fn {i, category} ->
      ...
      dot_color = Theme.get_color_style(color, "text", "fill")
      dot_bg_color = Theme.get_color_style(color, "background", "bg")
