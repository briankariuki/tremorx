defmodule Tremorx.Components.DatePicker do
  ...
  attr :day_selected_class, :string, doc: "CSS class to customize the selected day"
  attr :day_indicator_class, :string, doc: "CSS class to customize the selected day indicator"
  attr :day_in_range_class, :string, doc: "CSS class to customize the in range day"
  attr :nav_button_class, :string, doc: "CSS class to customize the date navigation bar buttons"
  attr :nav_class, :string, doc: "CSS class to customize the navigation container"
  attr :trigger_btn_class, :string, doc: "CSS class to customize the trigger button"
  ...
  def date_range_picker(assigns) do
    ...
    assigns =
      ...
      |> assign(
        cell_class:
          Tails.classes([
            Theme.make_class_name("datePicker", "cell"),
            "relative text-center focus-within:relative",
            "text-gray-900 dark:text-gray-50",
            if(is_nil(assigns[:cell_class]), do: "", else: assigns[:cell_class])
          ]),
        ...
        day_in_range_class:
          Tails.classes(
            day_class ++
              [
                Theme.make_class_name("datePicker", "inRangeDay"),
                "text-gray-900 dark:text-gray-50 bg-gray-100 dark:bg-gray-900 rounded-none",
                if(is_nil(assigns[:day_outside_class]), do: "", else: assigns[:day_outside_class])
              ]
          ),
        day_indicator_class:
        ...
        trigger_btn_class:
          Tails.classes([
            Theme.make_class_name("datePicker", "triggerBtn"),
            # base
            "max-w-xs peer flex w-full cursor-pointer appearance-none items-center gap-x-2 truncate rounded-md border px-3 py-2 shadow-sm outline-none transition-all sm:text-sm",
            # background color
            "bg-white dark:bg-gray-950",
            # border color
            "border-gray-300 dark:border-gray-800",
    ...
    <div class="max-w-xs w-full">
      <.menu
        class="mx-auto"
        menu_btn_class={@trigger_btn_class}
        menu_items_class="mt-2 min-w-fit -translate-x-1/2 left-[50%] right-[50%]"
      >
        <:button>
          <svg
            viewBox="0 0 24 24"
          ...
          <span className="flex-1 overflow-hidden text-ellipsis whitespace-nowrap text-left text-gray-900 dark:text-gray-50">
            <span id={"#{@id}_selected_range_trigger"} class="text-gray-400 dark:text-gray-600">
              <%= @placeholder %>
            </span>
          </span>
        </:button>
          ...
          <div
            class={
              Tails.classes([
                Theme.make_class_name("datePicker", "popover"),
                # base
                "relative z-50 w-full left-0 right-0 rounded-md border text-sm shadow-xl shadow-black/[2.5%]",
                # border color
                "border-gray-200 dark:border-gray-800",
                # background color
                "bg-white dark:bg-gray-950",
          ...
          >
            ...
            <div class="space-y-4 w-full">
              <date-range-picker
                ...
                day-outside-class={@day_outside_class}
                day-indicator-class={@day_indicator_class}
                day-in-range-class={@day_in_range_class}
              >
                <div class="flex w-full items-start">
                  <div class="p-3 flex-1 flex flex-col justify-center">
                    <div class="flex items-center mb-1 justify-between">
                      ...
                      <div class="size-8 invisible"></div>
                    </div>
                    ...
                    <table class={
                      ...
                      <thead
                        id={"#{@id}_prev_month_days"}
                        phx-update="ignore"
                        class="hidden gap-0 grid-cols-7"
                      >
                      </thead>
                      <tbody id={"#{@id}_prev_month_elements"} phx-update="ignore" class="hidden">
                      </tbody>
                    </table>
                  </div>

                  <div class="p-3 border-l border-gray-200 flex-1 flex flex-col justify-center">
                    <div class="flex items-center mb-1 justify-between">
                      <div class="size-8 invisible"></div>
                      <div
                        role="presentation"
                    ...
                    <table class={
                      ...
                      <thead
                        id={"#{@id}_next_month_days"}
                        phx-update="ignore"
                        class="hidden gap-0 grid-cols-7"
                      >
                      </thead>
                      <tbody id={"#{@id}_next_month_elements"} phx-update="ignore" class="hidden">
                      </tbody>
                    </table>
                  </div>
                </div>

                <div class="flex justify-between  items-center gap-x-2 border-t border-gray-200 p-3 dark:border-gray-800">
                  <div class="flex space-x-0.5">
                    <.text>Range:</.text>
                    <.text id={"#{@id}_selected_range"}></.text>
                  </div>
                  <div>
                    <.button id={"#{@id}_cancel_button"} variant="secondary" class="h-8" type="button">
                      Cancel
                    </.button>

                    <.button id={"#{@id}_apply_button"} variant="primary" class="h-8" type="button">
                      Apply
                    </.button>
                  </div>
                </div>
